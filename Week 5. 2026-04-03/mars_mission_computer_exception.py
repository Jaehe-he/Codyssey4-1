...
class DummySensor:
    ...
    def get_env(self):
        ...
        try:
            ...
            log_line = (
                f'{now}, '
                f"{self.env_values.get('mars_base_internal_temperature', 'N/A')}, "
                f"{self.env_values.get('mars_base_external_temperature', 'N/A')}, "
                f"{self.env_values.get('mars_base_internal_humidity', 'N/A')}, "
                f"{self.env_values.get('mars_base_external_illuminance', 'N/A')}, "
                f"{self.env_values.get('mars_base_internal_co2', 'N/A')}, "
                f"{self.env_values.get('mars_base_internal_oxygen', 'N/A')}\n"
            )

            # 파일 저장 예외 처리
            with open('mars_log_exception.txt', 'a') as f:
                f.write(log_line)
